// ============================================================
//  V4.3.3.6 – Robustes Portfolio
//  NEU: Neue Länderverteilung nach 6 Regionen:
//       Nordamerika · Europa · Schwellenländer · Pacific
//       · Sonstige ETF · Sonstige
// ============================================================
...
// ── PORTFOLIO STATE ───────────────────────────────────────────
const STORAGE_KEY  = 'portfolioV4336_setup';
const GLOBALS_KEY  = 'portfolioGlobalsV4336_setup';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updatePortfolioCountrySummary = () => {
        ...
        // Neue 6-Regionen-Logik ab V4.3.3.6
        const CLUSTER_DEFS = {
            'Nordamerika': {
                label: 'Nordamerika', color: '#1e40af',
                // USA, Kanada, Mexiko – Mexiko gilt hier als Nordamerika (NAFTA)
                countries: new Set(['USA','Vereinigte Staaten','Kanada','Nordamerika'])
            },
            'Europa': {
                label: 'Europa', color: '#047857',
                // Alle europäischen Länder – Achtung: Irland & Luxemburg als investierte Länder
                // werden NICHT hier gezählt (sie fallen unter „Sonstige ETF")
                countries: new Set(['Deutschland','Frankreich','Großbritannien','Niederlande','Österreich','Belgien','Schweiz','Schweden','Norwegen','Dänemark','Finnland','Spanien','Italien','Portugal','Europa','Polen','Tschechien','Ungarn','Rumänien','Griechenland','Litauen','Lettland','Estland','Bulgarien'])
            },
            'Schwellenländer': {
                label: 'Schwellenländer', color: '#7c3aed',
                // China, Indien, Lateinamerika und weitere klassische EM-Länder
                countries: new Set(['China','Volksrepublik China','Indien','Brasilien','Mexiko','Argentinien','Chile','Kolumbien','Peru','Türkei','Südafrika','Nigeria','Ägypten','Israel','Kasachstan','Namibia','Mauritius','Kuwait','Pakistan','Schwellenländer','Emerging Markets'])
            },
            'Pacific': {
                label: 'Pacific', color: '#0e7490',
                // Australien, Japan, Südkorea und weitere Pazifik-Länder (ohne generisches 'Asien')
                countries: new Set(['Japan','Australien','Neuseeland','Hongkong','Singapur','Taiwan','Südkorea','Korea','Thailand','Vietnam','Philippinen','Indonesien','Malaysia','Pacific'])
            },
            'Sonstige ETF': {
                label: 'Sonstige ETF', color: '#64748b',
                // Irland & Luxemburg als Ländergewichtung = ETF-Domizil, daher eigene Kategorie
                countries: new Set(['Irland','Luxemburg'])
            },
            'Sonstige': {
                label: 'Sonstige', color: '#94a3b8',
                // Alles, was keiner der obigen Kategorien zugeordnet werden kann;
                // inkl. 'Asien' (zu uneindeutig für Schwellenländer ODER Pacific)
                countries: new Set(['global','Global','sonstige','Sonstige','Sonstige Länder','Frontiers Markets','Frontier Markets','Asien'])
            }
        };
        const CLUSTER_ORDER = ['Nordamerika','Europa','Schwellenländer','Pacific','Sonstige ETF','Sonstige'];
                    ...
                    if (blockData) {
                        ...
                        if (fundData && fundData.countryWeightings) {
                            fundData.countryWeightings.forEach(cw => {
                                ...
                                if (!countryValues[cw.country]) countryValues[cw.country] = 0;
                                countryValues[cw.country] += v;
                            });
                        }
                    }
                }
        ...
        // Länder-Karte in der Info-Bar einblenden
        summaryContainer.style.display = 'block';

        // ── Cluster-Zuordnung ─────────────────────────────────────────
        const clusterValues = {};
        const clusterDetails = {};
        CLUSTER_ORDER.forEach(k => { clusterValues[k] = 0; clusterDetails[k] = {}; });

        Object.entries(countryValues).forEach(([country, val]) => {
            let assigned = false;
            for (const key of CLUSTER_ORDER) {
                if (CLUSTER_DEFS[key].countries.has(country.trim())) {
                    clusterValues[key] += val;
            ...
            if (!assigned) {
                // Wirklich nicht kategorisierbar → Sonstige
                clusterValues['Sonstige'] += val;
                clusterDetails['Sonstige'][country] = (clusterDetails['Sonstige'][country] || 0) + val;
            }
        });

        // ── Normalisierung auf 100% ──────────────────────────────────
        // Basis: Summe aller zugewiesenen Ländergewichte (nicht zwingend = totalInvestment,
        // da manche Fonds keine countryWeightings haben oder Gewichte ≠ 100% aufsummieren)
        ...
        CLUSTER_ORDER.forEach(clusterKey => {
            ...
            const top5 = Object.entries(clusterDetails[clusterKey])
                .map(([c, v]) => ({ c, p: (v / assignedTotal) * 100 }))
                .sort((a, b) => b.p - a.p).slice(0, 5);

            const top5Html = top5.map(d =>
                `<li style="display:flex;justify-content:space-between;margin-bottom:4px;gap:12px;">
                    <span>${d.c}</span><strong>${d.p.toFixed(1).replace('.', ',')}%</strong>
                </li>`
            ).join('');
            ...
            pill.innerHTML = `
                ...
                <div class="cluster-popup">
                    <div style="font-size:0.82em;font-weight:600;border-bottom:1px solid rgba(255,255,255,0.2);padding-bottom:5px;margin-bottom:7px;color:${cluster.color}">${cluster.label}</div>
    ...
    const renderFundList = funds => {
        ...
        funds.forEach(fund => {
            ...
            // NEU: Tooltip für Ländergewichtung
            let countryTooltipHtml = '';
            if (fund.countryWeightings && fund.countryWeightings.length > 0) {
                const listHtml = fund.countryWeightings.map(c => `<li>${c.country}: ${c.weight}%</li>`).join('');
                countryTooltipHtml = `
                    <div class="fund-country-tooltip">
                        ...
                        <div class="tooltip-content">
                            <strong style="font-size:1.05em; border-bottom:1px solid #555; display:block; padding-bottom:4px;">Top 5 Länder:</strong>
                            <ul>${listHtml}</ul>
                        </div>
                    </div>`;
            }
            ...
            li.innerHTML = `
                <div class="fund-info-wrapper">
                    <div class="fund-header" style="display:flex; align-items:center;">
